...
`move_todos` takes either `todoMoves`, a complete explicit list of card IDs and destination statuses, or `todoSelection`, a destination and validated AND conditions. Criteria are evaluated over the entire current board, not an AI-enumerated sample. Conditions support title, status, To Do text, notes and due date; an explicit `all` scope requires no conditions. The two selection modes cannot be combined. It supports subsets, whole lanes and mixed source/destination lanes. All targets are validated before one preview and one atomic card-only save. Unknown IDs, duplicate IDs, invalid conditions and invalid lanes reject the whole request. Already-correct cards are unchanged; unselected cards, task text, dates, notes and CRM records are preserved. Cancel, stale-preview protection and Undo use the same protections as other card changes.
