...
For browser QA, `node scripts/start-local-qa.js --offline` supplies the disposable `qa-reports@example.invalid` account with the fixture password documented in the local QA provider. Its exact simulated commands are `Compare total value under Ravi and Sarah`, `Compare values for Alpha QA, Beta QA and Gamma QA`, and `Now compare averages`. This launcher makes no real OpenAI or Xano requests.
