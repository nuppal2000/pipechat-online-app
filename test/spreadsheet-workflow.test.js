...
function harness(pick=async()=>({name:'Test.csv',matrix:[['ID','Value'],['001','25'],['002','0']],primary:0}),handler){
  ...
  const context={crypto,TextEncoder,AbortSignal,window:{PipelineCore:require('../public/pipeline-core.js'),PipeChatSchema:require('../public/table-schema.js'),PipeChatSheets:require('../public/spreadsheet-import.js'),PipeChatCsv:require('../public/csv-import.js'),PipeChatSpreadsheetUI:()=>({open:pick}),PipeChatIcons:{}},document:{getElementById:node,querySelector:()=>node('selector'),querySelectorAll:()=>[]},fetch:async(url,options)=>{calls.push({url,payload:JSON.parse(options.body)});const data=handler?await handler(url,calls.at(-1).payload):{...calls.at(-1).payload,updatedAt:'v2'};return {ok:true,json:async()=>data};},clearTimeout(){},setTimeout(){},sessionStorage:{removeItem(){},setItem(){}}};
