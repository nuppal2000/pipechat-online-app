...
function harness(handler=()=>({deals:[],customFields:[],tableSchema:{status:'pending'},updatedAt:'v2'})){
  ...
  const context={window:{PipelineCore:require('../public/pipeline-core.js'),PipeChatSchema:require('../public/table-schema.js'),PipeChatIcons:{}},document:{getElementById:node},sessionStorage:{removeItem:k=>storage.delete(k)},clearTimeout(){},fetch:async(url,options)=>{calls.push({url,options});return{ok:true,json:async()=>handler(url,options)};}};
