...
function snapshotResult(input) {
  ...
  let customFields;
  try {customFields=core.validateCustomFields(input.customFields);}
  ...
  const deals = input.deals.map(record => {
    ...
    try {
      for (const field of Object.keys(core.fields)) result[field] = core.validateStoredValue(field, record[field]);
      Object.assign(result,core.customValues(record,customFields));
  ...
  return { deals, updatedAt: input.updatedAt, ...(Object.hasOwn(input,'customFields')?{customFields}:{}) };
}
...
function createXanoBackend({ baseUrl, serverKey, fetchImpl = fetch, timeoutMs = 15000 } = {}) {
  ...
  return {
    ...
    async writeCrm(token, deals, expectedUpdatedAt, customFields) {
      if (expectedUpdatedAt === undefined) throw new BackendError('Refresh before saving. A CRM version is required.', 400);
      const data = snapshotResult({ deals, updatedAt: expectedUpdatedAt, ...(customFields===undefined?{}:{customFields}) });
      ...
      const result=snapshotResult(await request('crm', { method: 'PUT', token, body: { deals: data.deals, expectedUpdatedAt, ...(customFields===undefined?{}:{customFields:data.customFields}) } }));
