...
const routes = new Set([
  '/api/health', '/api/ready', '/api/monitor-status', '/api/auth/me', '/api/auth/signup', '/api/auth/login',
...
function monitorRequest(req, res, write = line => console.log(line), observe = () => {}) {
  ...
  function record(outcome) {
    ...
    try { observe(entry); } catch { /* Monitoring never changes the response. */ }
  }
