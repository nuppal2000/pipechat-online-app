    ...
    forRequest(req, res, { secureCookie = process.env.NODE_ENV === 'production' } = {}) {
      ...
      const adapter = {
        ...
        async writeCrm(ignoredToken, deals, expectedUpdatedAt, customFields, tableSchema, todoCards) {
          ...
          const data = snapshot({ deals, updatedAt: expectedUpdatedAt,...(todoCards===undefined?{}:{todoCards}),
          ...
          const result = snapshot(await rpc(todoCards===undefined?'pipechat_write_crm':'pipechat_write_workspace', {
            p_deals: data.deals, p_custom_fields: data.customFields, p_table_schema: data.tableSchema,
            p_expected_updated_at: expectedUpdatedAt,...(todoCards===undefined?{}:{p_todo_cards:data.todoCards})
          }));
          if (!isDeepStrictEqual(result.tableSchema, data.tableSchema) || !isDeepStrictEqual(result.customFields, data.customFields) ||
              !isDeepStrictEqual(result.deals, data.deals) || todoCards!==undefined&&!isDeepStrictEqual(result.todoCards,data.todoCards) || result.updatedAt === null || result.updatedAt === expectedUpdatedAt) {
