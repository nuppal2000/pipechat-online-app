...
function snapshotResult(input) {
  ...
  try {tableSchema=schemaCore.validate(input.tableSchema);table=core.create(tableSchema);customFields=table.validateCustomFields(input.customFields);schemaCore.validatePrimaryOrder(tableSchema,customFields);}
  ...
  let todoCards;
  try{todoCards=todoCore.validate(input.todoCards,deals,tableSchema,customFields);}catch{throw new BackendError('The storage backend returned invalid To Do cards.');}
  return { deals, updatedAt: input.updatedAt, ...(Object.hasOwn(input,'todoCards')?{todoCards}:{}), ...(Object.hasOwn(input,'customFields')?{customFields}:{}),...(tableSchema?{tableSchema}:{}) };
}
