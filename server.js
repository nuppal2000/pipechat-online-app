...
const todoInstructions = [
  ...
  'For moving MULTIPLE existing cards use move_todos with todoMoves containing EVERY matching card as {todoId,todoStatus}. Resolve IDs from the full pipeline.todoCards/todoView, not only the current lane or visible cards. Support subsets, all cards in a lane, and moves from different source lanes to the same or different destinations. Leave single-card properties and todos null. Do not move only the first match, combine tasks, add replacement cards, or alter card text, notes, dates or CRM cells. If the user explicitly says all matching tasks, include all; if a singular reference matches several cards, clarify using task text and title. The app validates the entire batch, previews each status transition, then saves atomically only after confirmation. update_todo remains available for a single card edit or move.',
...
actionSchema.required.push('todoMoves','clarificationOptions');
