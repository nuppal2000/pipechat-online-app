  ...
  const requestOptions = {
    ...
    body: JSON.stringify({
      ...
      instructions: spreadsheetBuild ? spreadsheetTypes.instructions : tableBuild ? tableSchemaCore.instructions : csv ? csvCore.instructions : tableSchema?.status==='ready' ? [
        ...
        "When the user explicitly asks to add a field or column, use add_field with newFieldName set to their requested label (for example Contact). This proposes one new text column for EVERY account, with all cells initially blank. It does not add an account or populate contact values. Ask for a name if none is given. Do not create duplicate fields or infer numeric/date types. A one-off unfamiliar attribute does not justify a suggestion; recurring concepts can use the separate propose_field action as described above. The app validates, previews, and requires confirmation before saving.",
