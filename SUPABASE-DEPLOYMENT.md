...
For first-column primary identity, apply `db/migrations/011-primary-column-order.sql` after migration 010 and before deploying this UI. It replaces the schema/custom-field validators without changing grants or rewriting stored records, cards, column order or usage. Explicit `columnOrder[0]` must identify an existing field and is the effective primary; the original field types and other semantic roles are preserved. A schema without explicit order retains its existing primary role. After a user removes a former primary-role field, an older validator may reject that valid ordered schema; use a matching release or a forward fix rather than replaying old validators.
