(function(root){
  ...
  function create({S,esc,icon,persistTodo,prepare,render,toast,localDate}){
    ...
    function draw(){
      ...
      $('addTodoBtn').disabled=disabled()||!S.records.length;$('todoSuggestion').hidden=true;
    }
    function openForm(){
      if(disabled())return;
      if(S.pending||S.clarification||draft?.dirty){toast('Preview or cancel the current card edit or proposal first.');return;}
      if(!S.records.length){toast('Add a record to the table first.');return;}
      dialogVersion=S.revision;$('todoDialogTitle').textContent='New To Do card';
      $('todoRecord').innerHTML=S.records.map(r=>`<option value="${r.id}">${esc(name(r.id))} / #${r.id}</option>`).join('');
    ...
    $('todoForm').addEventListener('submit',event=>{
      ...
      try{
        const card={...T.create('todo_'+crypto.randomUUID().replaceAll('-',''),Number($('todoRecord').value)),status:$('todoStatus').value,nextAction:$('todoNext').value,notes:$('todoNotes').value,dueDate:$('todoDue').value};
    ...
    function preview(p){
      ...
      $('trustBody').innerHTML=`<h3>${esc(name(p.recordId))}</h3>${!after?'<p class="error">Remove this card? The linked CRM record and all its information will be kept.</p>':[['Board status',after.status],['To Do',after.nextAction],['Notes',after.notes],['Due date',after.dueDate]].map(([label,value])=>`<div class="field-diff"><span>${label}</span><div class="todo-preview-value">${esc(value||'Not set')}</div></div>`).join('')}<div class="proposal-actions"><button class="${after?'primary':'danger'}" data-confirm ${S.saving?'disabled':''}>${after?'Confirm card':'Delete card'}</button><button class="secondary" data-cancel ${S.saving?'disabled':''}>Cancel</button></div>`;
    }
